...
doEvent.caribouCIP = function(sim, eventTime, eventType) {
  switch(
    ...
    init = {
      ...
      sim <- scheduleEvent(sim, P(sim)$.plotInitialTime, "caribouCIP", "plot")
      sim <- scheduleEvent(sim, P(sim)$.saveInitialTime, "caribouCIP", "save")
    },
    plot = {
      # ! ----- EDIT BELOW ----- ! #
      # do stuff for this event
      ...
      # e.g.,
      #sim <- scheduleEvent(sim, time(sim) + P(sim)$.plotInterval, "caribouCIP", "plot")
    ...
    save = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + P(sim)$.saveInterval, "caribouCIP", "save")
    ...
    event1 = {
      ...
      # e.g.,
      # sim <- scheduleEvent(sim, time(sim) + increment, "caribouCIP", "templateEvent")

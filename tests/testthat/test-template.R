...
#    `functionName` is a function's name in your module (e.g., `caribouCIPEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/Tati/Documents/GitHub/caribouCIP/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("caribouCIP")
  ...
  parameters <- list(
    ...
    caribouCIP = list(.saveInitialTime = NA)
  )
  ...
  if (exists("caribouCIPEvent1", envir = .GlobalEnv)) {
    simOutput <- caribouCIPEvent1(mySim)
  } else {
  ...
  if (exists("caribouCIPEvent2", envir = .GlobalEnv)) {
    simOutput <- caribouCIPEvent2(mySim)
  } else {
